...
<div class="footer">
  M. Scandola, NPsy-Lab.VR, Human Sciences Dept., UniVR <br>
  ciao ciao
</div>
...
* "Empirical" Bayes [@gelman2013bayesian]
...
curve(f(x), col="red", -4, 800, n=301, las=1)
```
...
sidebarPanel(
  ...
  "La linea rossa è la distribuzione da campionare, quella nera
  il risultato del campionamento.",
  width = 15
)
...
renderPlot({
  ...
  myMCMC_step <- function(old_point, density_function){
      new_point <- rnorm(1, mean = old_point, sd = 100)
  ...
  plot(density(x1[1:n]), main = "Sampling distribution", ylim = c(0,0.015))
  points(density(dat), xlim = c(min(x1),max(x1)), type = "l", lwd = 4, col = "red")
  rug(x1[1:n])
...
# Indici di correttezza della statistica Bayesiana
...
`r fa("envelope", fill = "steelblue")` michele.scandola\@univr.it <br>
